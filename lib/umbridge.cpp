...
std::vector<std::vector<double>> Model::Evaluate(const std::vector<std::vector<double>>& inputs,
                                                 json config_json) {
  (void)inputs;
...
std::vector<double> Model::Gradient(unsigned int outWrt,
                                    unsigned int inWrt,
...
std::vector<double> Model::ApplyJacobian(unsigned int outWrt,
                                         unsigned int inWrt,
                                         const std::vector<std::vector<double>>& inputs,
...
std::vector<double> Model::ApplyHessian(unsigned int outWrt,
                                        unsigned int inWrt1,
                                        unsigned int inWrt2,
                                        const std::vector<std::vector<double>>& inputs,
                                        const std::vector<double>& sens,
...
bool Model::SupportsEvaluate() { return false; }
bool Model::SupportsGradient() { return false; }
bool Model::SupportsApplyJacobian() { return false; }
bool Model::SupportsApplyHessian() { return false; }

std::string Model::GetName() const { return name; }

std::vector<std::string> Model::SupportedModels(std::string host, httplib::Headers headers) {
...
HTTPModel::HTTPModel(std::string host, std::string name, httplib::Headers headers)
...
std::vector<std::size_t> HTTPModel::GetInputSizes(const json& config_json) const {
  ...
  if (auto res = cli.Post("/InputSizes", headers, request_body.dump(), "application/json")) {
    ...
    std::vector<std::size_t> outputvec =
        response_body["inputSizes"].get<std::vector<std::size_t>>();
    return outputvec;
  } else {
...
std::vector<std::size_t> HTTPModel::GetOutputSizes(const json& config_json) const {
  ...
  if (auto res = cli.Post("/OutputSizes", headers, request_body.dump(), "application/json")) {
    json response_body = parse_result_with_error_handling(res);
    std::vector<std::size_t> outputvec =
        response_body["outputSizes"].get<std::vector<std::size_t>>();
    return outputvec;
  } else {
...
std::vector<std::vector<double>> HTTPModel::Evaluate(const std::vector<std::vector<double>>& inputs,
                                                     json config_json) {
...
std::vector<double> HTTPModel::Gradient(unsigned int outWrt,
                                        unsigned int inWrt,
                                        const std::vector<std::vector<double>>& inputs,
                                        const std::vector<double>& sens,
                                        json config_json) {
...
std::vector<double> HTTPModel::ApplyJacobian(unsigned int outWrt,
                                             unsigned int inWrt,
                                             const std::vector<std::vector<double>>& inputs,
                                             const std::vector<double>& vec,
                                             json config_json) {
  ...
  if (auto res = cli.Post("/ApplyJacobian", headers, request_body.dump(), "application/json")) {
    ...
    throw std::runtime_error("POST ApplyJacobian failed with error type '" +
                             to_string(res.error()) + "'");
  }
}

std::vector<double> HTTPModel::ApplyHessian(unsigned int outWrt,
                                            unsigned int inWrt1,
                                            unsigned int inWrt2,
                                            const std::vector<std::vector<double>>& inputs,
                                            const std::vector<double>& sens,
...
bool HTTPModel::SupportsEvaluate() { return supportsEvaluate; }
bool HTTPModel::SupportsGradient() { return supportsGradient; }
bool HTTPModel::SupportsApplyJacobian() { return supportsApplyJacobian; }
bool HTTPModel::SupportsApplyHessian() { return supportsApplyHessian; }
...
json parse_result_with_error_handling(const httplib::Result& res) {
  ...
  if (response_body.find("error") != response_body.end()) {
    throw std::runtime_error("Model server returned error of type " +
                             response_body["error"]["type"].get<std::string>() +
                             ", message: " + response_body["error"]["message"].get<std::string>());
  }
...
bool check_input_sizes(const std::vector<std::vector<double>>& inputs,
  ...
  if (inputs.size() != model.GetInputSizes(config_json).size()) {
    ...
    response_body["error"]["message"] =
        ...
        std::to_string(model.GetInputSizes(config_json).size()) + " but got " +
  ...
  for (std::size_t i = 0; i < inputs.size(); i++) {
    if (inputs[i].size() != model.GetInputSizes(config_json)[i]) {
      json response_body;
      response_body["error"]["type"] = "InvalidInput";
      response_body["error"]["message"] = "Input size mismatch! In input " + std::to_string(i) +
                                          " model expected size " +
                                          std::to_string(model.GetInputSizes(config_json)[i]) +
                                          " but got " + std::to_string(inputs[i].size());
      res.set_content(response_body.dump(), "application/json");
      res.status = 400;
      return false;
    }
...
bool check_sensitivity_size(const std::vector<double>& sens,
  ...
  if (sens.size() != model.GetOutputSizes(config_json)[outWrt]) {
    json response_body;
    response_body["error"]["type"] = "InvalidInput";
    response_body["error"]["message"] = "Sensitivity vector size mismatch! Expected " +
                                        std::to_string(model.GetOutputSizes(config_json)[outWrt]) +
                                        " but got " + std::to_string(sens.size());
    res.set_content(response_body.dump(), "application/json");
    res.status = 400;
    return false;
  }
...
bool check_vector_size(const std::vector<double>& vec,
  ...
  if (vec.size() != model.GetInputSizes(config_json)[inWrt]) {
    json response_body;
    response_body["error"]["type"] = "InvalidInput";
    response_body["error"]["message"] = "Vector size mismatch! Expected " +
                                        std::to_string(model.GetInputSizes(config_json)[inWrt]) +
                                        " but got " + std::to_string(vec.size());
    res.set_content(response_body.dump(), "application/json");
    res.status = 400;
    return false;
  }
...
bool check_output_sizes(const std::vector<std::vector<double>>& outputs,
  ...
  if (outputs.size() != model.GetOutputSizes(config_json).size()) {
    ...
    response_body["error"]["message"] =
        "Number of outputs declared by model does not match number of outputs "
        "returned by model. Model declared " +
        std::to_string(model.GetOutputSizes(config_json).size()) + " but returned " +
  ...
  for (std::size_t i = 0; i < outputs.size(); i++) {
    if (outputs[i].size() != model.GetOutputSizes(config_json)[i]) {
      json response_body;
      response_body["error"]["type"] = "InvalidOutput";
      response_body["error"]["message"] = "Output size mismatch! In output " + std::to_string(i) +
                                          " model declared size " +
                                          std::to_string(model.GetOutputSizes(config_json)[i]) +
                                          " but returned " + std::to_string(outputs[i].size());
      res.set_content(response_body.dump(), "application/json");
      res.status = 500;
      return false;
    }
...
bool check_input_wrt(int inWrt,
  ...
  if (inWrt < 0 || inWrt >= (int)model.GetInputSizes(config_json).size()) {
    ...
    response_body["error"]["type"] = "InvalidInput";
    response_body["error"]["message"] =
        "Input inWrt out of range! Expected between 0 and " +
        std::to_string(model.GetInputSizes(config_json).size() - 1) + " but got " +
        std::to_string(inWrt);
    res.set_content(response_body.dump(), "application/json");
    res.status = 400;
    return false;
...
bool check_output_wrt(int outWrt,
  ...
  if (outWrt < 0 || outWrt >= (int)model.GetOutputSizes(config_json).size()) {
    ...
    response_body["error"]["type"] = "InvalidInput";
    response_body["error"]["message"] =
        "Input outWrt out of range! Expected between 0 and " +
        std::to_string(model.GetOutputSizes(config_json).size() - 1) + " but got " +
        std::to_string(outWrt);
    res.set_content(response_body.dump(), "application/json");
    res.status = 400;
    return false;
  }
...
void log_request(const httplib::Request& req, const httplib::Response& res) {
  std::cout << "Incoming request from: " << req.remote_addr << " | Type: " << req.method << " "
...
void serveModels(std::vector<Model*> models,
  ...
  httplib::Server svr;
  std::mutex model_mutex; // Ensure the underlying model is only called
                          // sequentially
  ...
  svr.Post("/Evaluate", [&](const httplib::Request& req, httplib::Response& res) {
    ...
    if (model_lock.owns_lock()) {
      model_lock.unlock(); // for safety, although should unlock after request
                           // finished
    }
