...
bool waitForHQJobState(const std::string &job_id, const std::string &state) {
  const std::string command =
      "hq job info " + job_id + " | grep State | awk '{print $4}'";
  ...
  do {
    ...
    if (job_status == "" || (state != "FINISHED" && job_status == "FINISHED") ||
        job_status == "FAILED" || job_status == "CANCELED") {
      std::cerr << "Wait for job status failure, status : " << job_status
                << std::endl;
      return false;
    }
...
void umbridge::LoadBalancer::submitHQJob(const std::string &model_name,
                                         bool force_default_submission_script) {
  ...
  const std::filesystem::path submission_script_model_specific(
      "job_" + model_name + ".sh");

  std::string hq_command = "hq submit --output-mode=quiet ";
  if (std::filesystem::exists(submission_script_dir /
                              submission_script_model_specific) &&
      !force_default_submission_script) {
    hq_command +=
        (submission_script_dir / submission_script_model_specific).string();
  } else if (std::filesystem::exists(submission_script_dir /
                                     submission_script_generic)) {
